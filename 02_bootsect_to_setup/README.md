...
`rep movsw` 是 x86 字符串指令的**块数据搬运**组合：

| 指令 | 作用 |
|------|------|
| `movsw` | Move String Word，从 **`DS:SI`** 搬运 **2 字节** 到 **`ES:DI`**，然后 `SI += 2`、`DI += 2` |
| `rep` | Repeat，重复执行 `movsw` 直到 `CX` 减到 0，每执行一次 `CX--` |

> 🔗 **与前文联动**：这里的 `DS` 和 `ES` 正是 `start2` 中分别设为 `0x7C0` 和 `0x9000` 的寄存器。`movsw` 是**隐含操作数**指令，它不写死地址，而是自动读取当时的段寄存器。如果 `DS`/`ES` 设错了，数据就会搬错地方。

**计算验证**：`CX = 256` × 每次 2 字节 = **512 字节** = 恰好一个扇区。

> 💡 这相当于 C 语言里的 `memcpy(dst=0x90000, src=0x7C00, size=512)`，只不过用硬件字符串指令一次性完成，比手写循环更高效。
...
| `DS` | Data Segment | **读数据**时用 `DS:SI`、`DS:BX` 等 |
| `ES` | Extra Segment | **字符串操作**的目的段（如 `movsw` 的 `ES:DI`） |
| `SS` | Stack Segment | **栈操作**时用 `SS:SP` |

> 📐 **栈顶物理地址公式**：`SS × 16 + SP`
>
> 初始时 `SS:SP = 0x9000:0xFF00`，所以**栈顶**在 `0x90000 + 0xFF00 = 0x9FF00`。x86 栈是向下增长的，`push` 时 `SP` 减小，栈向低地址方向生长。
>
> 💡 **为什么要统一为 `0x9000`？** 搬家完成后，代码、数据、栈全在 `0x90000 ~ 0x9FFFF` 这同一个 64KB 段内。如果不统一，CPU 访问数据或栈时会"走错门"（比如 `DS` 还指向旧的 `0x7C0`，读到的就是旧地址的垃圾数据）。
...
load_setup:
    ...
    jnc ok_load_setup    ; CF = 0 表示成功，跳转到 ok_load_setup
```
...
> 🔗 **与前文联动**：这里的 `ES` 正是 `go:` 标签处设为 `0x9000` 的寄存器。`BX = 0x0200` 是一个偏移量设计，bootsect 占了 `0x90000 ~ 0x901FF`，setup 必须紧接在后面从 `0x90200` 开始放。所以 `ES:BX = 0x9000:0x0200 = 0x90200 = SETUPSEG:0`。
...
| 指令 | 全称 | 条件 | 行为 |
|------|------|------|------|
| `jc` | Jump if Carry | `CF = 1` | 出错时跳转 |
| `jnc` | Jump if **Not** Carry | `CF = 0` | 成功时跳转 |

BIOS 中断执行完毕后，通过 **FLAGS 寄存器** 中的 `CF`（Carry Flag，进位标志）返回结果：
...
int 0x13 读盘
   ↓
CF = 0 ? ──是──→ jnc 跳转到 ok_load_setup（继续执行）
   ↓ 否
CF = 1 → 不跳转，继续执行下面的错误处理（重置磁盘 → 重试）
```

- 如果读盘失败（`CF = 1`），不跳转，执行 `int 0x13 AH=0` 重置磁盘，然后 `jmp load_setup` 无限重试

**为什么 `int 0x13` 出现两次，行为却不同？**

`int 0x13` 是 BIOS 磁盘服务的**总入口**，具体干什么由 **`AH`（功能号）** 决定：

| | 第一次 `int 0x13` | 第二次 `int 0x13` |
|--|------------------|------------------|
| **AH 值** | `0x02` | `0x00` |
| **功能** | 读磁盘扇区到内存 | 重置磁盘驱动器 |
| **所需参数** | AL, CH, CL, DH, DL, ES:BX | 只需要 DL |

> 💡 就像打电话转分机，同一个号码 `0x13`，拨 `0x02` 转分机“读盘服务”，拨 `0x00` 转分机“重置服务”。

**为什么失败处理里还要重新设置 `DX = 0x0000`？**

理论上 `int 0x13` 读盘不会修改 DX，但这里仍然重新设置，原因有二：

1. **防御性编程**：BIOS 由各厂商自行实现，虽然标准说“不该碰 DX”，但万一某个奇葩 BIOS 在出错时改了 DX 呢？每次调用前显式设置参数，不依赖之前的值。
2. **代码自解释**：`movw $0x0000, %dx` 让读者一眼就知道重置磁盘需要 `DL = 0`（0号驱动器），不需要回头看 `load_setup` 开头的设置。

#### 6️⃣ 打印成功信息

**`int 0x10` —— BIOS 视频服务总入口**

和 `int 0x13` 一样，`int 0x10` 也是一个**多功能中断**，由 **`AH`（功能号）**决定具体行为：

| AH | 功能 | 说明 |
|----|------|------|
| `0x00` | 设置显示模式 | 切换分辨率、颜色数 |
| `0x02` | 设置光标位置 | 把光标移到指定行列 |
| `0x0E` | ⭐ **Teletype 输出字符** | **本章用的就是这个** |
| `0x13` | 写字符串 | 批量输出一串字符（书上原版用的这个） |

**`AH = 0x0E` 的工作方式**：

- `AL` = 要显示的字符
- BIOS 把字符打到屏幕上，**光标自动后移一格**
- 遇到 `0x0D`（`\r`）光标回到行首
- 遇到 `0x0A`（`\n`）光标移到下一行

> 💡 `AH = 0x0E` 只设置一次，后面每次 `int 0x10` 都是同一个功能，只是 `AL` 里的字符在变。
...
- **`lodsb` 的隐含操作数**：它自动从 **`DS:SI`** 读取一个字节到 `AL`，然后 `SI++`。此时 `DS = 0x9000`（由前文 `go` 标签处统一设置），所以实际读的是 `0x90000 + msg_offset`，也就是搬家后的字符串位置。如果 `DS` 还是旧的 `0x7C0`，这里就会读到原位置的垃圾数据
- **`$msg` 是偏移地址**：`movw $msg, %si` 中的 `$msg` 是 msg 标签相对于当前段的**偏移地址**，不是完整物理地址。`lodsb` 实际读取的是 `DS:SI = 0x9000:msg_offset`。汇编器编译时只能确定段内偏移，真正的物理地址必须靠段寄存器配合计算
...
**为什么 `.org 508` 能确保签名在正确位置？**

`.org 508` 是汇编器指令，意思是"后面的数据从偏移 508 开始放"。如果前面的代码+数据不到 508 字节，汇编器会**自动用零填充**，把 508~511 的位置留出来：

| 偏移 | 内容 | 说明 |
|------|------|------|
| 0 ~ 507 | 代码+数据 / 填充 0x00 | bootsect 实际内容 |
| 508 ~ 509 | `root_dev`（2 字节） | 根设备号 |
| 510 ~ 511 | `0x55 0xAA` | ⭐ BIOS 检查的引导签名 |

> ⚠️ **小端序注意**：x86 是小端序，`.word 0xaa55` 在内存里实际存的是 `0x55`（低字节）在前，`0xAA`（高字节）在后。所以第 510 字节是 `0x55`，第 511 字节是 `0xAA`，BIOS 读到的顺序正好是 `0xAA55`。
...
```asm
    ...
    movb $0x0E, %ah
setup_print_loop:
    lodsb                ; ⭐ 同样使用 DS:SI 读字符串，此时 DS = 0x9020
    testb %al, %al
    jz setup_print_done
    int $0x10
...
- **再次看到 `lodsb`**：setup 中也用 `lodsb` 读字符串，依赖 `DS` 寄存器。所以 `_start_setup` 开头第一件事就是把 `DS` 初始化为 `0x9020`
...
| 阶段 | 名称 | 谁决定 | 作用 |
|------|------|--------|------|
| **链接时** | **链接地址** | `ld -Ttext 0x0` | 符号的虚拟地址，影响 `$msg` 的值 |
| **加载时** | **加载地址** | BIOS / bootsect 自己 | 代码实际被放到内存的哪个物理位置 |
| **运行时** | **运行地址** | `CS/DS/ES` 段寄存器 | CPU 执行时看到的地址 |

`-Ttext 0x0` 告诉链接器：**所有符号地址都按从 0 开始算偏移**。这样 `$msg` 才是一个**纯偏移量**（如 `0x20`），而不是一个**绝对物理地址**（如 `0x7C20`）。

**举例：假设 msg 在文件第 32 字节处**

**情况一：`-Ttext 0x0`（正确做法）**

```
链接器认为：_start 在 0x0，msg 在 0x20
生成指令：movw $0x20, %si    ← $msg = 0x20（纯偏移量）
```

运行时：
```
DS = 0x9000, SI = 0x20
物理地址 = 0x90000 + 0x20 = 0x90020  ✅ 正确！
```

**情况二：`-Ttext 0x7C00`（错误做法）**

```
链接器认为：_start 在 0x7C00，msg 在 0x7C20
生成指令：movw $0x7C20, %si  ← $msg = 0x7C20（包含了加载地址）
```

运行时：
```
DS = 0x9000, SI = 0x7C20
物理地址 = 0x90000 + 0x7C20 = 0x97C20  ❌ 读到完全错误的位置！
```

> 💡 汇编里的 `BOOTSEG = 0x7C0` 是**运行时**的段地址，和链接器没关系。链接器负责"算偏移"，段寄存器负责"加基址"，两者分工明确。
